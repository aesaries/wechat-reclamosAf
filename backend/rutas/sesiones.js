...
router.post("/desde-whatsapp", (req, res) => {
  ...
  let usuario = db
    .prepare("SELECT * FROM usuarios WHERE telefono = ?")
    .get(telefono);
  if (!usuario) {
    const info = db
      .prepare("INSERT INTO usuarios (telefono) VALUES (?)")
      .run(telefono);
    usuario = db
  ...
  // 2.5. Chequeamos timeout de inactividad (15 min)
  const TIMEOUT_MS = 15 * 60 * 1000; // 15 minutos
  if (sesion && sesion.ultima_actividad) {
    const ultima = new Date(sesion.ultima_actividad).getTime();
    const diff = Date.now() - ultima;

    if (diff > TIMEOUT_MS) {
      console.log(
        `⏰ Sesión ${sesion.token.slice(0, 8)}... reseteada por inactividad (${Math.round(diff / 60000)} min)`,
      );

      db.prepare(
        `
      UPDATE sesiones
      SET estado_conversacion = 'inicio',
          datos_temporales = NULL,
          ultima_actividad = ?
      WHERE id = ?
    `,
      ).run(new Date().toISOString(), sesion.id);

      // Recargamos la sesión con los datos actualizados
      sesion = db.prepare("SELECT * FROM sesiones WHERE id = ?").get(sesion.id);
    }
  }
  ...
  if (!sesion) {
    ...
    sesion = db
      .prepare("SELECT * FROM sesiones WHERE id = ?")
      .get(info.lastInsertRowid);
  }

  // Actualizamos ultima_actividad
  db.prepare("UPDATE sesiones SET ultima_actividad = ? WHERE id = ?").run(
...
router.post("/:token/avanzar", (req, res) => {
  ...
  if (!sesion) {
    return res
      .status(404)
      .json({ ok: false, error: "Sesión inválida o expirada" });
  }
  ...
  if (sesion.datos_temporales) {
    try {
      temporales = JSON.parse(sesion.datos_temporales);
    } catch (e) {
      temporales = {};
    }
  }
...
router.post("/:token/finalizar", (req, res) => {
  ...
  if (sesion.datos_temporales) {
    try {
      datos = JSON.parse(sesion.datos_temporales);
    } catch (e) {
      datos = {};
    }
  }
